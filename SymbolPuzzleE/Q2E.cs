                ...
                for (var StartRow = Row + 2; StartRow >= Row; StartRow--)
                {
                    for (var StartColumn = Column - 2; StartColumn <= Column; StartColumn++)
                    {
                        try
                        {
                            ...
                            CellList.Add(GetCell(StartRow, StartColumn));
                            // change
                            CellList.Add(GetCell(StartRow, StartColumn + 1));
                            CellList.Add(GetCell(StartRow, StartColumn + 2));
                            CellList.Add(GetCell(StartRow - 1, StartColumn + 2));
                            CellList.Add(GetCell(StartRow - 2, StartColumn + 2));
                            CellList.Add(GetCell(StartRow - 2, StartColumn + 1));
                            CellList.Add(GetCell(StartRow - 2, StartColumn));
                            CellList.Add(GetCell(StartRow - 1, StartColumn));
                            CellList.Add(GetCell(StartRow - 1, StartColumn + 1));
                            // end change
                            foreach (var P in AllowedPatterns)
                            {
                                ...
                                {
                                    // change
                                    foreach (var theCell in CellList)
                                    {
                                        // use the function CheckSymbolAllowed
                                        // 
                                        if (theCell.CheckSymbolAllowed(CurrentSymbol) == false)
                                        {
                                            Console.WriteLine("you cannot overlap patterns");
                                            return 0;
                                        }
                                    }
                                    // end change
