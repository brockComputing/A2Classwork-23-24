        ...
        class Puzzle
        {
            ...
            public virtual int AttemptPuzzle()
            {
                ...
                while (!Finished)
                {
                    ...
                    Console.WriteLine($"You have {SymbolsLeft} symbols left"); // change
                    bool Valid = false;
                    int Row = -1;
                    while (!Valid)
                    {
                    ...
                    string removeAns = Console.ReadLine();
                    if (removeAns == "y")
                    {
                        Cell CurrentCell = GetCell(Row, Column);
                        if (CurrentCell.GetSymbol() == "@")
                        {
                            Console.WriteLine("this is a blocked cell @");
                        }
                        else if(CurrentCell.CheckSymbolAllowed(CurrentCell.GetSymbol()) == false)
                        {
                            Console.WriteLine("this is part of a pattern");
                        }
                        else
                        {
