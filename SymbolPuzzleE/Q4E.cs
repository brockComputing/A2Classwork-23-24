    ...
    class PuzzleCS
    {
        ...
        class Puzzle
        {
            ...
            public Puzzle(int Size, int StartSymbols)
            {
                ...
                AllowedSymbols = new List<string>();
                // change
                int qCount = Rng.Next(1, 4); int xCount = Rng.Next(1, 4); int tCount = Rng.Next(1, 4);
                Pattern QPattern = new Pattern("Q", "QQ**Q**QQ",qCount);
                AllowedPatterns.Add(QPattern);
                AllowedSymbols.Add("Q");
                Pattern XPattern = new Pattern("X", "X*X*X*X*X", xCount);
                AllowedPatterns.Add(XPattern);
                AllowedSymbols.Add("X");
                Pattern TPattern = new Pattern("T", "TTT**T**T", tCount); // end change
            ...
            public virtual int AttemptPuzzle()
            {
                ...
                while (!Finished)
                {
                    ...
                    //change
                    foreach (var item in AllowedPatterns)
                    {
                        Console.WriteLine(item.OutputPatternCount());
                    }//end change
                    // same as above using classic for loop
                    for (int i = 0; i < AllowedPatterns.Count; i++)
                    {
                        Console.WriteLine(AllowedPatterns[i].OutputPatternCount());
                    } // end change
                    bool Valid = false;
                    int Row = -1;
                    while (!Valid)
        ...
        class Pattern
        {
            ...
            //change
            public Pattern(string SymbolToUse, string PatternString, int count)
            {
                Symbol = SymbolToUse;
                PatternSequence = PatternString;
                PatternCount = count;
            ...
            {
                if (SymbolPlaced != Symbol || PatternCount == 0) // change
                {
                    return false;
                }
            ...
            // change
            public string OutputPatternCount()
            {
                return $"{Symbol}'s left is {PatternCount}";
            } // end change
